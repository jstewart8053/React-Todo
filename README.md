...
- `<TodoList />`(GETS ARRAY AND ITENERATES OVER LIST AND SENDS TO TODO FOR RENDER) receives your Todos array and iterates over the list generating a new `<Todo />` for each element in the array.
- `<Todo />`(RENDERS THE TODO DATA) is a component that takes in the `todo` data and displays the task to the screen.
